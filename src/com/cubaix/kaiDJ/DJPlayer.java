...
public class DJPlayer extends TimedCanvas implements BasicPlayerListener, KJDigitalSignalProcessor{
	...
	int destCard = 0;
	public int playState = -1;// -1 = nothing, 0 = pause, 1 = playing, 2 = fadeout, 3 = fadein
	Map openProperties;
	Map currentProperties;
	double currentGain = 0.8;

	protected String currentAuthor = "";
	...
	GC dblBufGC;
	public Rectangle progessBarR = new Rectangle(0, 0, 0, 0);
	public Rectangle volumeBarR = new Rectangle(0, 0, 0, 0);
	 ...
	 * @param style
	 */
	public DJPlayer(KaiDJ aParentKDJ, Composite parent, int style, int aDest,boolean aUseDSP) {
	...
	public void load(String aPath) {
		try {
			...
			if(new File(aPath+".kai").exists()) {
				kaiSrt = new KaiSrt(aPath);
			}
			else {
				kaiSrt = null;
			}
			
			parentKDJ.logoPanel.needRedrawSlow();
		} catch (Throwable t) {
			t.printStackTrace(System.err);
		}
	}
	...
	public void stateUpdated(BasicPlayerEvent event) {
		...
		if (event.getCode() == BasicPlayerEvent.EOM) {
			// Be sure this is the active player
			if(playState == 1 && currentGain > 0.1) {
				timeToNext();
			}
			playState = -1;
		}
	...
	void loadImg() {
		...
		InputStream aIS = DJPlayer.class.getResourceAsStream("img/playerBck.png");
		playerBck = new Image(parentKDJ.display, aIS);
		aIS = DJPlayer.class.getResourceAsStream("img/playerBck.png");
